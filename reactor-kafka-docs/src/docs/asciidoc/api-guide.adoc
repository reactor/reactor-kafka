...
the maximum number of in-flight messages can also be configured before the Sender is created.
...
Let's now create a sequence of records to send to Kafka. Each `SenderRecord` consists of a Kafka
https://kafka.apache.org/0101/javadoc/org/apache/kafka/clients/producer/ProducerRecord.html[ProducerRecord]
and a correlation metadata for matching send results to requests. `ProducerRecord` consists of a key/value pair
...
all records published on `records`. If `records` is a non-terminating `Flux`, send continues to send
...
acknowledged. Only offsets explictly acknowledged using `ReceiverOffset#acknowledge()` are committed.
...
can disable periodic commits and explictly invoke `ReceiverOffset#commit()` when required to trigger
...
committing an offset acknowledges and commits all previous offsets on that partition. All
acknowledged offsets are committed when partitions are revoked during rebalance and when the receive
Flux is terminated.
...
Applications which dont require offset commits to Kafka may disable automatic commits by not acknowledging
