...
without providing correlation metadata using the send method on an `SenderOutbound` instance obtained using
`Sender#outbound()`.
...
The send sequence is initiated by subscribing to the Mono obtained from `SenderOutbound#then()`.
...
Multiple sends can be chained together using a sequence of sends on the returned `SenderOutbound`.
When the Mono returned from `SenderOutbound#then()` is subscribed to, the sends are invoked
