...
results may be different from the order of records published. Results are delivered in order for each partition,
but results from different partitions may be interleaved. In the sample the message index is included as
correlation metadata to match each result to its corresponding record.
